...
public class MainCode extends Utils {
    ...
    public void main(List<String> mods, List<String> packs, String mc) throws IOException, URISyntaxException {
        ...
        if (new File(mc + "/launcher_profiles.json").exists()) {
            installProfile("launcher_profiles.json", mc);
        }
        if (new File(mc + "/launcher_profiles_microsoft_store.json").exists()) {
            installProfile("launcher_profiles_microsoft_store.json", mc);
        }
        ...
        if (numbers.listFiles() != null && Objects.requireNonNull(numbers.listFiles()).length != 1) {
            FileUtils.deleteDirectory(numbers);
            numbers.mkdir();
        }
